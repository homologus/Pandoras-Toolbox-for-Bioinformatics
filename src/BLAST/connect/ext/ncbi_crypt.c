/* $Id: ncbi_crypt.c 424437 2014-01-15 22:44:09Z lavr $
 * ===========================================================================
 *
...
struct SCRYPT_KeyTag {
    int           seed;  /*store it first so we can hack it in the tests :-)*/
    short         off1;
    short         off2;
    unsigned long magic;
...
void CRYPT_Free(CRYPT_Key key)
{
    ...
    if (key->magic != CRYPT_MAGIC) {
        CORE_LOG_X(1, eLOG_Critical, "[CRYPT_Free]  Magic corrupted");
    }
    free(key);
}
...
extern char* CRYPT_EncodeString(CRYPT_Key key, const char* str)
{
    ...
    if (key->magic != CRYPT_MAGIC) {
        CORE_LOG_X(3, eLOG_Critical, "[CRYPT_Encode]  Bad key magic");
        return 0;
    }
    if (!str) {
        return 0;
    ...
    len = strlen(str);
    version = s_Version;
    assert(0 <= version  &&  (size_t) version < SizeOf(s_Coders));
...
extern char* CRYPT_DecodeString(CRYPT_Key key, const char* str)
{
    ...
    if (key->magic != CRYPT_MAGIC) {
        CORE_LOG_X(5, eLOG_Critical, "[CRYPT_Decode]  Bad key magic");
        return 0;
    }
    ...
    version = *str++ - '0';
    if (version & ~1) {
        CORE_TRACEF(("[CRYPT_Decode]  Unknown crypt version `%u'", version));
        errno = EINVAL;
        return 0;
    }
    assert(0 <= version  &&  (size_t) version < SizeOf(s_Coders));
