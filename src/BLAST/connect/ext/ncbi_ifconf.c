/* $Id: ncbi_ifconf.c 430177 2014-03-24 17:14:08Z lavr $
 * ===========================================================================
 *
...
{
    /* IANA range for Auto Private IP Addressing (APIPA): 169.254/16 */
    const SIPRange k_AutoDHCP = { eIPRange_Network, 0xA9FE0000, 0xFFFF0000 };
    unsigned int a, b;
    struct ifconf ifc;
    struct ifreq ifr;
    char *buf, *p;
    ...
    for (p = buf;  p < buf + ifc.ifc_len;  p += size) {
        ...
        if (b != INADDR_ANY) {
            a = ip;
#ifdef SIOCGIFMTU
            mtu = ioctl(s, SIOCGIFMTU, &ifr) < 0 ? 0
                : ifr.ifr_mtu > 0 ? (size_t) ifr.ifr_mtu : 0;
#endif /*SIOCGIFMTU*/
            if (!NcbiIsInIPRange(&k_AutoDHCP, ip))
                break;
        }
    }
