...
main: {
    ...
    if ($aln_method eq 'bowtie') {
        ...
        if ($left && $right) {
            ...
            $bowtie_cmd = "bowtie2 " . $aligner_params{"${aln_method}_${est_method}"} . " $read_type -X $max_ins_size -x $db_index_name -1 $left -2 $right -p $thread_count | samtools view -S -b -o $bam_file -";
        }
        else {

            $bowtie_cmd = "bowtie2 " . $aligner_params{"${aln_method}_${est_method}"} . " $read_type -x $db_index_name -U $single -p $thread_count | samtools view -S -b -o $bam_file -";
        }
    }
