...
/* $Id: ncbi_lbsm.h 437834 2014-06-10 20:10:12Z lavr $
 * ===========================================================================
 *
...
#define LBSM_DEFAULT_DUMPFILE        LBSM_DEFAULT_HOMEDIR "run/lbsmd"
...
typedef struct LBSM_PACKED {
    ...
    unsigned int   kernel;        /* kernel id (8, 8, 16)[/SP]            12 */
...
typedef struct LBSM_PACKED {
    ...
    unsigned short sid;           /* site ID?                            128 */
 ...
 * SLBSM_Host::joined field.  "items" can be used to improve performance by
 * stopping after having dropped the specified number of records (0 means to
 * do all).
 */
...
/* Set penalty (expressed in percentages of unavailability) via LBSM daemon, or
 * rerate the given service (LBSM_RERATE_DEFAULT to set default rate per conf,
 * LBSM_RERATE_RESERVE to reserve the server, which makes it unavailable for
 * most uses, or 0 to turn the service off) so that every LBSM host will have
 * the information soon.  Return 0 if submission failed;  non-zero otherwise.
 */
int/*bool*/ LBSM_SubmitPenaltyOrRerate
(const char*           name,  /* [in] service name to be penalized/rerated   */
